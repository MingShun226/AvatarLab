-- Safe Migration Fix for Avatar Creation Errors
-- This script safely handles existing tables and adds missing columns

-- =============================================
-- 1. CREATE OR UPDATE AVATAR_TEMPLATES TABLE
-- =============================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'avatar_templates' AND column_name = 'is_active') THEN
        ALTER TABLE avatar_templates ADD COLUMN is_active BOOLEAN DEFAULT TRUE;
    END IF;

    -- Update check constraint to include all template types
    ALTER TABLE avatar_templates DROP CONSTRAINT IF EXISTS avatar_templates_template_type_check;
    ALTER TABLE avatar_templates ADD CONSTRAINT avatar_templates_template_type_check
        CHECK (template_type IN ('backstory', 'hidden_rules', 'personality', 'voice'));
END $$;
...
-- =============================================
-- 3. DROP EXISTING CONFLICTING STORAGE POLICIES
-- =============================================
DO $$
BEGIN
    -- Drop existing policies that might conflict
    DROP POLICY IF EXISTS "Authenticated users can upload knowledge files" ON storage.objects;
    DROP POLICY IF EXISTS "Users can access own knowledge files" ON storage.objects;
END $$;

-- =============================================
-- 4. CREATE STORAGE POLICIES FOR KNOWLEDGE-BASE BUCKET
-- =============================================
...
-- =============================================
-- 5. CREATE AVATAR KNOWLEDGE FILES TABLE
-- =============================================
...
-- =============================================
-- 8. ADD MISSING COLUMNS TO AVATARS TABLE
-- =============================================
DO $$
BEGIN
...
-- =============================================
-- 9. INSERT SAMPLE TEMPLATE DATA (SAFE)
-- =============================================
DO $$
BEGIN
    -- Only insert if no templates exist
    IF NOT EXISTS (SELECT 1 FROM avatar_templates LIMIT 1) THEN
        -- Insert backstory templates
        INSERT INTO avatar_templates (template_type, title, content, description, category, is_active) VALUES
        ('backstory', 'Professional Assistant', 'You are a professional AI assistant with expertise in business and productivity. You have a formal yet approachable communication style and always strive to provide accurate, helpful information.', 'A professional assistant template for business use', 'business', true),
        ('backstory', 'Creative Companion', 'You are a creative and imaginative AI companion who loves art, writing, and innovation. You approach problems with creativity and always encourage artistic expression and out-of-the-box thinking.', 'A creative companion template for artistic projects', 'creative', true),
        ('backstory', 'Technical Expert', 'You are a technical expert with deep knowledge in programming, engineering, and technology. You provide detailed technical explanations and help solve complex technical problems with precision.', 'A technical expert template for development work', 'technical', true),
        ('backstory', 'Friendly Teacher', 'You are a patient and encouraging teacher who loves helping people learn. You break down complex topics into simple, understandable parts and celebrate every learning milestone.', 'A teaching template for educational purposes', 'education', true);

        -- Insert hidden rules templates
        INSERT INTO avatar_templates (template_type, title, content, description, category, is_active) VALUES
        ('hidden_rules', 'Professional Boundaries', 'Always maintain professional boundaries. Do not share personal information or engage in inappropriate conversations. Focus on the task at hand and provide value-driven responses.', 'Professional boundary guidelines', 'business', true),
        ('hidden_rules', 'Creative Freedom', 'Encourage creative expression while maintaining helpfulness. Be open to unconventional ideas but ensure they are constructive and positive.', 'Guidelines for creative interactions', 'creative', true),
        ('hidden_rules', 'Technical Accuracy', 'Always prioritize technical accuracy over speed. If uncertain about technical details, acknowledge limitations and suggest proper resources for verification.', 'Technical accuracy guidelines', 'technical', true),
        ('hidden_rules', 'Safe Learning', 'Create a safe learning environment where mistakes are learning opportunities. Never make learners feel inadequate for not knowing something.', 'Safe learning environment guidelines', 'education', true);
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Avatar templates count: %', (SELECT COUNT(*) FROM avatar_templates);
    RAISE NOTICE 'Storage buckets: %', (SELECT COUNT(*) FROM storage.buckets WHERE id IN ('avatars', 'knowledge-base'));
END $$;