...
import { useMarketplace, usePurchases } from '@/hooks/useDatabase';
import { useAuth } from '@/hooks/useAuth';

const MarketplaceSectionNew = () => {
  ...
  // Use new database hooks instead of localStorage
  const { user } = useAuth();
  const { marketplaceAvatars, loading: marketplaceLoading, loadMarketplaceAvatars } = useMarketplace();
  const { purchasedAvatars, purchaseAvatar, loading: purchaseLoading } = usePurchases(user?.id);

  useEffect(() => {
    // Load marketplace avatars with filters
    loadMarketplaceAvatars({
      search: searchQuery || undefined,
      category: filter !== 'all' ? filter : undefined,
      priceRange: priceRange !== 'all' ? priceRange : undefined
    });
  }, [filter, searchQuery, priceRange, loadMarketplaceAvatars]);
  ...
  const filteredAvatars = marketplaceAvatars.filter(avatar => {
    if (filter !== 'all' && avatar.creator_studio && !avatar.creator_studio.toLowerCase().includes(filter.toLowerCase())) {
      return false;
    }
          ...
          <Card key={avatar.id} className="group hover:shadow-lg transition-shadow cursor-pointer">
            <CardContent className="p-0">
              ...
              <div className="relative overflow-hidden rounded-t-lg">
                <img
                  src={avatar.avatar_images ? JSON.parse(avatar.avatar_images)[0] : '/placeholder-avatar.png'}
                  alt={avatar.name}
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {avatar.personality_traits && JSON.parse(avatar.personality_traits).slice(0, 2).map((trait: string) => (
...
export default MarketplaceSectionNew;